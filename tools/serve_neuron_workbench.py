...
class GenerationJob:
    def __init__(self, *, app_dir: Path, payload: dict) -> None:
        self.job_id = uuid.uuid4().hex[:12]
        self.app_dir = app_dir.resolve()
        self.output_app_dir = self.app_dir / "generated_runs" / safe_run_id(str(payload.get("run_id") or "current_review_pipeline"))
...
def generation_labels(payload: dict) -> tuple[str, str]:
...
def load_run(app_dir: Path, run_id: str) -> dict | None:
...
def run_generation_params(run: dict | None) -> dict:
    ...
    for stage in normalize_pipeline(run.get("pipeline") or []):
        ...
        if stage_id == "temporal_highpass_gaussian" and "sigma_frames" in stage_params:
            ...
            params["sigma_label"] = f"{int(float(stage_params['sigma_frames'])):02d}" if float(stage_params["sigma_frames"]).is_integer() else f"{round(float(stage_params['sigma_frames']) * 10):03d}"
...
def infer_dataset_id(app_dir: Path, review_data: dict | None = None) -> str:
    params = (review_data or {}).get("parameters") or {}
    return str(params.get("datasetId") or app_dir.parent.name)


def find_raw_video(review_data: dict, dataset_id: str) -> Path | None:
...
def generated_dataset_manifest(app_dir: Path, payload: dict, *, output_app_dir: Path | None = None) -> Path:
...
def ensure_run_record(app_dir: Path, run_id: str, dataset_id: str, status: str, *, output_app_dir: Path | None = None, output_root: Path | None = None) -> None:
    path = app_dir / "architecture_runs.json"
...
class WorkbenchHandler(BaseHTTPRequestHandler):
    ...
    def _send_json(self, status: int, payload: dict | list, *, include_body: bool = True) -> None:
